<script lang="ts">
    ...
    import type {IOptions} from "@/types/components/item.d";
    import type {IConfig} from "@/types/config/default.d";

    import type PluginBlinkoSyncHelper from "@/index";
    import {checkAccessToken, test} from "@/main";

    // Svelte 5/4 兼容 props 声明
    export let plugin: InstanceType<typeof PluginBlinkoSyncHelper>;
    export let config: IConfig;
    export let notebookOptions: IOptions = [];

    // 调试：检查 props
    console.log("setting.svelte 组件初始化");
...
>
    <Panel display={PANELS[0].key === focusPanel}>
        ...
        <Item
                title="日记目标标题"
                text="指定同步到日记中的标题下。格式: '标题名' (严格匹配标题文本)。例如: 'Blinko' 只匹配标题文本完全等于'Blinko'。留空或未匹配到则追加到文档末尾。仅在同步方案为'日记'时生效"
                isShow={showUseTemplate}
        >
            <Input
                    slot="input"
                    type={itemType.text}
                    settingKey="dailyNoteHeading"
                    settingValue={config.base.dailyNoteHeading}
                    placeholder="例如: ## Blinko (严格匹配)"
                    on:changed={e => {
                        config.base.dailyNoteHeading = e.detail.value;
                        updated();
                    }
                }
            />
    ...
    <Panel display={PANELS[1].key === focusPanel}>
        ...
        <Item
                title="显示创建时间"
                text="显示笔记的创建时间和修改时间。格式: 创建时间 / 修改时间(如: 2025-10-08 22:30:41 / 2025-10-08 22:33:44)。如果未修改则只显示创建时间"
                isTip={true}
